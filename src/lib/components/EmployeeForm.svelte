<script lang="ts">
	...
	// Form data - reativo para atualizar quando employee mudar
	$: formData = {
		name: employee?.name || '',
		...
		dismissalDate: employee?.dismissalDate ? new Date(employee.dismissalDate).toISOString().split('T')[0] : ''
	};
	...
	function validateField(field: string, value: any) {
		...
		const requiredFields = ['name', 'abbreviation', 'email', 'phone', 'address', 'number', 'neighborhood', 'city', 'state', 'zipCode', 'documentNumber', 'position', 'hireDate'];
	...
	function handleSubmit() {
		...
		if (!submitData.dismissalDate) submitData.dismissalDate = null;
	...
	<div class="relative top-4 mx-auto p-5 border w-11/12 max-w-4xl shadow-lg rounded-md bg-white" on:click|stopPropagation>
		...
		<div class="mb-6">
			...
			<p class="mt-1 text-sm text-gray-600">
				{isEdit ? 'Altere os dados do funcionário abaixo.' : 'Preencha os dados do novo funcionário.'}
			</p>
		</div>
		...
		<form on:submit|preventDefault={handleSubmit} class="space-y-6">
			...
			<div class="bg-gray-50 p-4 rounded-lg">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<!-- Abbreviation -->
					<div>
						<label for="abbreviation" class="block text-sm font-medium text-gray-700">Abreviatura *</label>
						<input
							type="text"
							id="abbreviation"
							bind:value={formData.abbreviation}
							on:blur={() => validateField('abbreviation', formData.abbreviation)}
							maxlength="10"
							class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 {errors.abbreviation ? 'border-red-300' : ''}"
							placeholder="Ex: JS"
						/>
			...
			<!-- Job Information -->
			<div class="bg-gray-50 p-4 rounded-lg">
				<h4 class="text-md font-medium text-gray-900 mb-4">Informações Profissionais</h4>
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					<!-- Position -->
					<div>
						<label for="position" class="block text-sm font-medium text-gray-700">Função *</label>
						<input
							type="text"
							id="position"
							bind:value={formData.position}
							on:blur={() => validateField('position', formData.position)}
							class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 {errors.position ? 'border-red-300' : ''}"
					...
					<!-- Hire Date -->
					<div>
						<label for="hireDate" class="block text-sm font-medium text-gray-700">Data de Admissão *</label>
						<input
							type="date"
							id="hireDate"
							bind:value={formData.hireDate}
							on:blur={() => validateField('hireDate', formData.hireDate)}
							class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 {errors.hireDate ? 'border-red-300' : ''}"
						/>
						{#if errors.hireDate}
							<p class="mt-1 text-sm text-red-600">{errors.hireDate}</p>
						{/if}
					</div>

					<!-- Dismissal Date -->
					<div>
						<label for="dismissalDate" class="block text-sm font-medium text-gray-700">Data de Demissão</label>
						<input
							type="date"
							id="dismissalDate"
							bind:value={formData.dismissalDate}
							class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
						/>
					</div>
				</div>
