<script lang="ts">
	...
	$: {
		...
		// Adicionar item de funcionários para Admin e Manager
		const employeeItems = (user?.role === 'ADMIN' || user?.role === 'MANAGER') ? [
			{
				name: 'Funcionários',
				href: '/employees',
		...
		allNavigationItems = user?.role === 'ADMIN' 
			? [...navigationItems, ...employeeItems, ...companyItems, ...adminItems] 
			: [...navigationItems, ...employeeItems, ...companyItems];
	}
	...
	const navigationItems = [
		...
		{
			name: 'Lançamentos de Caixa',
			href: '/cash-transactions',
			icon: 'M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12s-1.536-.219-2.121-.659c-1.172-.879-1.172-2.303 0-3.182C10.464 7.781 11.232 8 12 8s1.536.219 2.121.659'
		},
		{
			name: 'Relatório de Aluguel',
			href: '/reports',
